...
function getWithRetry (uri, logContext, idempotencyKey) {
  let started
  let lastRetry
    ...
    return request({
      ...
      .catch((err) => {
        ...
        if (err.statusCode > 499 || err.code === 'ETIMEDOUT') {
          console.log(`Client ${logContext.counter}. Request failed: retry, Will run for ${number + 1}. time`)
          lastRetry = Date.now()
          retry(err)
        }
...
function call (idempotencyKey = uuidv1()) {
  counter += 1
